...
import wget
from utils import *
from base_models import AlexNet, C3DNet, convert_to_fcn, C3DNet2, SIMPLE_CNN, Time2Vec, C3D_SPORTS, ModelTrunk_2
...
class ActionPredict:
            ...
            for imp, b, p in zip(seq, bbox_seq[i], pid):
                ...
                if os.path.exists(img_save_path) and not regen_data:
                    ...
                    if crop_type == 'none':
                        ...
                        # resized_im = cv2.addWeighted(resized_im, 0.5, resized_im, 0.5, 0) # todo?
    ...
    def get_model(self, data_params, **kwargs):
        """
        Generates a model
        Args:
...
class TST_VISION(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        x = self._multi_self_attention(name='enc0_' + data_types[0], representation_size=attention_size, input_shape=(16, 768), **transformer_params)(x)
        encoder_outputs.append(x)
        x = self.normlayer(name='norm1_'+data_types[0], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[1])
        x = self._multi_self_attention(name='enc1_' + data_types[1], representation_size=attention_size, input_shape=(16, 768), **transformer_params)(x)
        encoder_outputs.append(x)
        ...
        net_model.summary()
        plot_model(net_model, to_file='TST_VISION.png')
        return net_model
...
class PCPA_TR_FIXTTE(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        x = self.normlayer(name='norm0_'+data_types[0], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[0])
        x = self._multi_self_attention(name='enc0_' + data_types[0], representation_size=attention_size)(x)
        encoder_outputs.append(x)
        x = self.normlayer(name='norm1_'+data_types[1], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[1])
        x = self._multi_self_attention(name='enc1_' + data_types[1], representation_size=attention_size)(x)
        encoder_outputs.append(x)
        x = self._rnn(name='enc2_' + data_types[2], r_sequence=return_sequence)(network_inputs[2])
        current = [x, network_inputs[3]]
        x = Concatenate(name='concat_early3', axis=2)(current)
...
class PCPA_TR_VISUAL(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        encoder_outputs.append(x)
        x = self.normlayer(name='norm0_'+data_types[1], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[1])
...
class TST_3(ActionPredict):
    ...
    def __init__(self,
        ...
        self._multi_self_attention_text = ModelTrunk_3b
        self.normlayer = BatchNormalization
    ...
    def get_model(self, data_params):
        ...
        x = self._multi_self_attention_text(name='enc2_' + data_types[2], representation_size=attention_size, input_shape=network_inputs[2].shape[1:], **transformer_params)(x)
        encoder_outputs.append(x)
        x = self.normlayer(name='norm3_'+data_types[3], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[3])
        x = self._multi_self_attention_text(name='enc3_' + data_types[3], representation_size=attention_size, input_shape=network_inputs[3].shape[1:], **transformer_params)(x)
        encoder_outputs.append(x)
        x = self.normlayer(name='norm4_'+data_types[4], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[4])
        x = self._multi_self_attention_text(name='enc4_' + data_types[4], representation_size=attention_size, input_shape=network_inputs[4].shape[1:], **transformer_params)(x)
        encoder_outputs.append(x)
...
class C3D_TRANSFORMER(ActionPredict):
    """
    later TST on text+images
    Class init function
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
        plot_model(net_model, to_file='C3D_TRANSFORMER.png')
        return net_model


class LATER_TST_2(ActionPredict):
    """
    later TST on text+images
    Class init function
    ...
    def __init__(self,
        ...
        self._rnn_cell = GRUCell if cell_type == 'gru' else LSTMCell
        self._3dconv = C3DNet if self._backbone == 'c3d' else I3DNet
        self._multi_self_attention = ModelTrunk_2
        self.normlayer = BatchNormalization
    ...
    def get_model(self, data_params):
        ...
        x = Lambda(lambda y: K.expand_dims(y, axis=1))(x)
        encoder_outputs.append(x)
        x = self.normlayer(name='norm3_'+data_types[3], axis=-1, momentum=0.99, epsilon=0.00001)(network_inputs[3])
        x = self._multi_self_attention(name='enc3_' + data_types[3], representation_size=attention_size, input_shape=network_inputs[3].shape[1:], include_dense_0=False, **transformer_params)(x)
        x = Lambda(lambda y: K.expand_dims(y, axis=1))(x)
        encoder_outputs.append(x)
        x = self.normlayer(name='norm4_'+data_types[4], axis=-1, momentum=0.99, epsilon=0.00001)(network_inputs[4])
        x = self._multi_self_attention(name='enc4_' + data_types[4], representation_size=attention_size, input_shape=network_inputs[4].shape[1:], include_dense_0=False, **transformer_params)(x)
        x = Lambda(lambda y: K.expand_dims(y, axis=1))(x)
        encoder_outputs.append(x)
...
class TST_VISION_MEAN(ActionPredict):
    """
    hierfusion PCPA_MULTI
    Class init function
...
class TST_HYBRID_NONVISUAL(ActionPredict):
    """
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
        plot_model(net_model, to_file='TST_HYBRID.png')
        return net_model
