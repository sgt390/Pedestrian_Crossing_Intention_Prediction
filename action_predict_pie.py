...
class MASK_PCPA_4_2D_TTE(ActionPredict):
    """
    hierfusion MASK_PCPA
    Class init function
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
        plot_model(net_model, to_file='MASK_PCPA_4_2D_TTE.png')
        return net_model
...
class MASK_PCPA_4_2D_NORM(ActionPredict):
    ...
    def __init__(self,
        ...
        self._rnn_cell = GRUCell if cell_type == 'gru' else LSTMCell
        self._3dconv = C3DNet if self._backbone == 'c3d' else I3DNet
        self._normlayer = BatchNormalization
    ...
    def get_model(self, data_params):
        ...
        x = self._normlayer(name='norm0_'+data_types[0], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[0])
        x = self._rnn(name='enc0_' + data_types[0], r_sequence=return_sequence)(network_inputs[0])
        encoder_outputs.append(x)
        encoder_outputs.append(x)
        x = self._normlayer(name='norm1_'+data_types[1], axis=-1, momentum=0.99, epsilon=0.0001)(network_inputs[1])
        x = self._rnn(name='enc1_' + data_types[1], r_sequence=return_sequence)(network_inputs[1])
        encoder_outputs.append(x)
        x = self._rnn(name='enc2_' + data_types[2], r_sequence=return_sequence)(network_inputs[2])
        current = [x, network_inputs[3]]
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
        plot_model(net_model, to_file='MASK_PCPA_4_2D_BATCH.png')
        return net_model
...
class MASK_PCPA_4_2D_PIE(ActionPredict):
    """
    hierfusion MASK_PCPA_4_PIE
    Class init function
    ...
    def get_data(self, data_type, data_raw, model_opts):
        # assert model_opts['obs_length'] == 16 # todo ??
        model_opts['normalize_boxes'] = False
        self._generator = model_opts.get('generator', False)
        data_type_sizes_dict = {}
        process = model_opts.get('process', True)
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
        plot_model(net_model, to_file='MASK_PCPA_4_2D_PIE.png')
        return net_model